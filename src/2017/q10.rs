...
fn b() {
  ...
  assert_eq!(vec![64, 7, 255].iter().map(|x| format!("{:02x}", x)).join(""), "4007ff");
  assert_eq!(get_input("1,2,3"), vec![49,44,50,44,51,17,31,73,47,23]);
  assert_eq!(densify(&process_data(256, &get_input(""), 64)), "a2582a3a0e66e6e86e3812dcb672a272");
  assert_eq!(densify(&process_data(256, &get_input("AoC 2017"), 64)), "33efeb34ea91902bb2f59c9920caa6cd");
  assert_eq!(densify(&process_data(256, &get_input("1,2,3"), 64)), "3efbe78a8d82f29979031a4aa0b16a9d");
  assert_eq!(densify(&process_data(256, &get_input("1,2,4"), 64)), "63960835bcdc130f0b66d7ff4f6a5a8e");
}
