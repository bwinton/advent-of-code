...
use combine::{one_of, many1, Parser};
...
impl Jump {
    pub fn build(s: &str) -> InstructionResult {
        let result = string("jmp ").with(
                one_of("+-".chars()).and(many1::<String, _>(digit()))
            )
            .parse(s).map(|x| x.0);
        match result {
            Ok((sign, value)) => {
...
impl Instruction for JumpEven {
    fn execute(&self, cpu: &mut CPU) {
        cpu.pc += if cpu.registers[&self.register] % 2 == 0 { self.offset - 1 } else { 0 }
    }
}
...
impl Instruction for JumpOne {
    fn execute(&self, cpu: &mut CPU) {
        cpu.pc += if cpu.registers[&self.register] == 1 { self.offset - 1 } else { 0 }
    }
}
...
impl Display for CPU {
    fn fmt(&self, f: &mut Formatter) -> Result {
        ...
        for (pc, inst) in self.instructions.iter().enumerate() {
            writeln!(f, "{} {}", if (pc as i64) == self.pc { "->" } else { "  "}, inst)?
        }
        Ok(())
    }
}
