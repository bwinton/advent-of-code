...
fn process_data_a(data: &str) -> String {
    ...
    for line in data.lines() {
        if let Some(cap) = RE.captures(line) {
            ...
            let mut dependencies = dep_graph.entry(cap[2].chars().next().unwrap()).or_insert(vec![]);
...
fn get_timing(data: &str, workers: usize, delay: i32) -> i32 {
    ...
    while nodes.len() > 0 {
        ...
        for worker in workers.iter().filter(|(current, time)| time <= &0 && current != &char::MAX) {
            for value in dep_graph.values_mut() {
...
#[test]
fn a() {
    assert_eq!(process_data_a("Step C must be finished before step A can begin.
...
#[test]
fn b() {
    assert_eq!(get_timing("Step C must be finished before step A can begin.
Step C must be finished before step F can begin.
Step A must be finished before step B can begin.
Step A must be finished before step D can begin.
Step B must be finished before step E can begin.
