...
fn parse(data: &str) -> (HashMap<(i64, i64), Cell>, (i64, i64)) {
    ...
    for (y, line) in data.lines().enumerate() {
        ...
        for (x, cell) in line.chars().enumerate() {
            if let Some(cell) = match cell {
                ...
                board.insert((x as i64, y as i64), cell);
            }
        }
    }
    (board, max)
...
fn get_energized_count(
    mut curr: Vec<((i64, i64), Direction)>,
    board: &HashMap<(i64, i64), Cell>,
    max: (i64, i64),
) -> usize {
        ...
        for pos in curr {
            ...
            match (board.get(&pos.0), pos.1) {
                ...
                | (Some(Cell::SplitterHorizontal), Direction::West) => {
                    if let Some(cell) = pos.1.move_pos(pos.0, 1, Some((0, 0)), Some(max)) {
                        next.push((cell, pos.1));
                    }
                }
                (Some(Cell::SplitterHorizontal), _) => {
                    if let Some(cell) = Direction::East.move_pos(pos.0, 1, Some((0, 0)), Some(max))
                    {
                        next.push((cell, Direction::East));
                    };
                    if let Some(cell) = Direction::West.move_pos(pos.0, 1, Some((0, 0)), Some(max))
                    {
                        next.push((cell, Direction::West));
                    };
                }
                (Some(Cell::SplitterVertical), _) => {
                    if let Some(cell) = Direction::North.move_pos(pos.0, 1, Some((0, 0)), Some(max))
                    {
                        next.push((cell, Direction::North));
                    };
                    if let Some(cell) = Direction::South.move_pos(pos.0, 1, Some((0, 0)), Some(max))
                    {
